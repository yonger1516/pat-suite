...
#scalefactor=40000000000 #(4TB)
#scalefactor=50000000000 #(5TB)
#scalefactor=60000000000 #(6TB)
#scalefactor=100000000000 #(10TB)

#OUTPUT_HDFS=Terasort/Input-1TB
#OUTPUT_HDFS=dtap://TenantStorage/Terasort/Input-1TB
OUTPUT_HDFS=HiBench/Terasort/Input
...
PARAMS="$PARAMS -Ddfs.blocksize=512M"

PARAMS="$PARAMS -Dmapreduce.map.memory.mb=16384"
#PARAMS="$PARAMS -Dmapreduce.map.cpu.vcores=1"
PARAMS="$PARAMS -Dmapreduce.map.java.opts=-Xmx14400m"
PARAMS="$PARAMS -Dmapreduce.task.io.sort.mb=12800"
PARAMS="$PARAMS -Dmapreduce.task.io.sort.factor=100"
PARAMS="$PARAMS -Dmapreduce.map.sort.spill.percent=0.99"
PARAMS="$PARAMS -Dmapreduce.reduce.memory.mb=3430"
PARAMS="$PARAMS -Dmapreduce.reduce.java.opts=-Xmx3200m"
...
#teragen specific default 2 maps, it should be override
#PARAMS="$PARAMS -Dmapreduce.job.maps=1536"
PARAMS="$PARAMS -Dmapreduce.job.maps=234"
...
#su - hdfs -c "$HADOOP_EXECUTABLE fs -rm -r -skipTrash $OUTPUT_HDFS"
...
START=$(date +%s)
set -x
#su - hdfs -c "$HADOOP_EXECUTABLE jar $HADOOP_EXAMPLES_JAR teragen $PARAMS $scalefactor $OUTPUT_HDFS"
$HADOOP_EXECUTABLE jar $HADOOP_EXAMPLES_JAR teragen $PARAMS $scalefactor $OUTPUT_HDFS
set +x
END=$(date +%s)
DURATION=$((END-START))

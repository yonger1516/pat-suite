...
#INPUT_HDFS=dtap://TennatStorage/benchmarks/TestDFSIO/io_data
#OUTPUT_HDFS=dtap://TenantStorage/benchmarks/TestDFSIO/io_data
...
START=$(date +%s)
set -x
$HADOOP_EXECUTABLE jar $HADOOP_TEST_JAR TestDFSIO $PARAMS -write -nrFiles $nrFiles -size ${fileSize}KB 
set +x
END=$(date +%s)
DURATION=$((END-START))

...
PARAMS="$PARAMS -Dmapreduce.map.memory.mb=1400"
#PARAMS="$PARAMS -Dmapreduce.map.java.opts=-Xmx1300m"

#tuning intermediate storage 
PARAMS="$PARAMS -Dmapreduce.task.io.sort.mb=680"
...
$HADOOP_EXECUTABLE jar $HADOOP_EXAMPLES_JAR terasort $PARAMS -Dmapreduce.map.java.opts="-Xmx1300m -Xms1300m" -Dmapreduce.reduce.java.opts="-Xmx1850m -Xms1850m" $INPUT_HDFS $OUTPUT_HDFS
set +x
END=$(date +%s)
DURATION=$((END-START))
